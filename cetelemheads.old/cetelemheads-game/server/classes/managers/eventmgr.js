...
EventMgr.prototype = {
  ...
  onGameEventTake: function(client, data) {
    ...
    var taker = this._game.findPlayerByUid(data.takerUid);
    if (taker == null) {
    ...
    for (var index in targets) {
      ...
      this._game.sendToAll(effectActivateEvent, {
    ...
    this._eventEffectExpirationTimeout[event.name] = setTimeout(function() {
      for (var index in targets) {
        ...
        that._game.sendToAll(effectDeactivateEventName, {
          targetUid: target.uid,
